...
//Library instance and initialization count.
...
static HBITMAP rghBmpStandardImages[3];
 ...
 *  and calls FInitialize for real initialization.
 */
...
BOOL FInitialize(HANDLE hInstance)
   ...
   #ifdef WIN32
    //Only perform this initialization once in Win32
    if (0!=gcInit)
        return TRUE;
   #endif
...
int WINAPI _WEP(int bSystemExit)
    {
    if (0==--gcInit)
        {
        CursorsFree();
...
static BOOL ToolButtonInit(void)
    ...
    //Win3.0 doesn't support COLOR_BTNHIGHLIGHT, so leave it white.
    if (0x0300 < (UINT)GetVersion())
        rgbHi=GetSysColor(COLOR_BTNHIGHLIGHT);
    else
        rgbHi=crStandard[STDCOLOR_WHITE];

    hBrushDither=HBrushDitherCreate(GetSysColor(COLOR_BTNFACE)
        , rgbHi);
...
void ToolButtonFree(void)
    ...
    if (NULL!=hDCMono)
        {
        DeleteDC(hDCMono);
        ODSu("Info:  hDCMono=%X safely deleted.", hDCMono);
        }
    ...
    if (NULL!=hDCGlyphs)
        {
        DeleteDC(hDCGlyphs);
        ODSu("Info:  hDCGlyphs=%X safely deleted.", hDCGlyphs);
        }
    ...
    if (NULL!=hBrushDither)
        ...
        ODSu("Info:  hBrushDither=%X safely deleted.", hBrushDither);
        }
    ...
    for (i=0; i < 3; i++)
        ...
        if (NULL!=rghBmpStandardImages[i])
            ...
            ODSu("Info:  image bitmap=%X safely deleted."
                , rghBmpStandardImages[i]);
            }
...
static HBRUSH HBrushDitherCreate(COLORREF rgbFace
    , COLORREF rgbHilight)
    {
...
BOOL WINAPI UIToolButtonDraw(HDC hDC, int x, int y, int dx, int dy
    ...
    {
    return UIToolButtonDrawTDD(hDC, x, y, dx, dy, hBmp, bmx, bmy
        , iImage , uStateIn, &tdd);
    }
 ...
 *  buttons.
 *
 *  This is the same as UIToolButtonDraw but adds the pTDD
 *  configuration structure.  UIToolButtonDraw calls us with that
 *  pointing to the default 96dpi structure.
 *
 * Parameters:
 *  hDC             HDC on which to draw.
 *  x, y            int coordinates at which to draw.
...
BOOL WINAPI UIToolButtonDrawTDD(HDC hDC, int x, int y, int dx, int dy
    ...
    /*
     * Draw a button sans image.  This also fills crSys with the
    ...
    DrawBlankButton(hDC, x, y, dx, dy
        , (BOOL)(uState & BUTTONGROUP_DOWN), crSys);
    ...
    if ((uState & BUTTONGROUP_ACTIVE)
        ...
        if (PRESERVE_ALL==uColors || fColorsSame)
            ...
            BitBlt(hMemDC, 0, 0, bmx, bmy, hDCGlyphs, iImage*bmx
                , 0, SRCCOPY);
            ...
            for (i=STDCOLOR_BLACK; i<=STDCOLOR_WHITE; i++)
                ...
                BitBlt(hDCMono, 0, 0, bmx, bmy, hDCGlyphs, iImage*bmx
                    , 0, SRCCOPY);
                ...
                if (NULL!=hBR)
                    ...
                    if (NULL!=hObj)
                        {
                        BitBlt(hMemDC, 0, 0, dx-1, dy-1, hDCMono, 0
                            , 0, ROP_DSPDxax);
    ...
    if ((uState & BUTTONGROUP_DISABLED
        ...
        if (NULL!=hBR)
            ...
            if (NULL!=hObj)
                ...
                BitBlt(hDC, x, y, dx-2, dy-2, hDCMono, 0, 0
                    , ROP_PSDPxax);
                SelectObject(hDC, hObj);
                }
    ...
    if (uState & BUTTONGROUP_LIGHTFACE)
        ...
        if (NULL!=hObj)
            ...
            BitBlt(hDC, x+1, y+1, dx-i, dy-i, hDCMono, 0, 0
                , ROP_DSPDxax);
            SelectObject(hDC, hObj);
            }
 ...
 *  offset in the global hDCMono.  Anywhere in the image that you
...
static void MaskCreate(UINT iImage, int dx, int dy, int bmx, int bmy
    ...
    BitBlt(hDCMono, xOffset, yOffset, bmx, bmy, hDCGlyphs
